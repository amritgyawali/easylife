...
- **SPA/route rewrites:** the static export (`web.output: "static"` in `app.json`) pre-renders every route to its own HTML file, so a catch-all SPA rewrite is not required for the routes that exist. It does, however, need `"cleanUrls": true` in `vercel.json` (already set) — routes with no child routes export as a flat `<route>.html` file (e.g. `sign-in.html`, `settings/data.html`) rather than `<route>/index.html`, and without `cleanUrls` Vercel 404s on the clean path (`/sign-in`) while happily serving the same content at `/sign-in.html`. This was live-broken once already (surfaced as the app looking permanently stuck on its loading spinner, since the post-sign-out/session-check redirect target 404'd) — confirmed by curling every exported route directly. If you add a dynamic route later (e.g. `/notes/[id]`) that needs client-side-only resolution, add a `vercel.json` rewrite for that specific pattern rather than a blanket catch-all, to avoid breaking the pre-rendered static routes.
- **Security headers:** `vercel.json` (repo root) sets `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, a `Permissions-Policy`, and a `Content-Security-Policy` whose `connect-src`/`img-src` are scoped to `https://*.supabase.co` (+ `wss://` for realtime). The CSP keeps `'unsafe-inline'` on `script-src`/`style-src` because the Expo static export and React Native Web inject inline bootstrap script and styles; tighten to nonces/hashes only if you move off the static export. It also allows `blob:` (and `data:`) in `connect-src` and `blob:` in `frame-src`, which the Reader and the document upload path both need: a file picked or dropped in the browser is read back through `fetch()` on its `blob:`/`data:` URL, and a PDF is displayed by embedding that same URL. Both are local, same-document sources — they can't be used to reach a third-party origin — but dropping either directive breaks reading and uploading **only on the deployed site**, so verify against the real headers rather than a local dev server.
- **Cache rules:** `vercel.json` caches the content-hashed bundler output under `/_expo/static/` and `/assets/` as `public, max-age=31536000, immutable`, and forces route HTML to `max-age=0, must-revalidate` so a new deploy is picked up immediately.
