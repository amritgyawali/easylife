...
**Durable outbox (quick-capture is bulletproof).** The quick-add flow (tasks and notes) writes through a persisted outbox — `src/services/offline/outbox-store.ts` saves each write (entity, op, payload) to AsyncStorage, and `sync-runner.ts` drains it to Supabase on reconnect, on queue changes, and on a slow retry interval. Because the row's `id` is client-generated, the write is applied as an idempotent upsert keyed on that id, so a retry or a replay after restart can never duplicate. This is the piece TanStack's in-memory pausing can't do (a paused mutation can't carry its function across a reload): a task captured on a dead train and then force-closed still syncs on next launch. Adding another offline-capable entity is one line in `OUTBOX_TABLES`.

**Still a hardening step:** the *existing detailed edit screens* (not quick-add) still use direct TanStack mutations, which are session-durable (queued and auto-synced on reconnect within a session) but not replayed across a hard restart. Routing those through the same outbox is the remaining generalisation. The schema decisions this all depends on (soft deletion, `version`, `device_id` — see [DATABASE.md](./DATABASE.md)) were in place from Phase 1.
