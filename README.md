...
> - **Reader** — a full-page reader (`/reader`) for **any document**: drop or pick a file and read it in the app without downloading it, or open anything already in the vault through a short-lived private link.
>   - **Word, PowerPoint, OpenDocument (.odt/.odp), EPUB, HTML, RTF and Markdown** are parsed into formatted text — headings, bold/italic, links, numbered and bulleted lists, quotes, code and tables.
>   - **Excel, OpenDocument spreadsheets and CSV/TSV** open as real grids, with a sheet switcher, spreadsheet column labels and Excel dates decoded.
>   - **ZIP archives** list their contents; **PDFs** embed in the page; **images** zoom and rotate; **audio and video** play inline on web.
>   - Everything supports find-with-highlighting and match stepping, and misnamed files are identified by their contents rather than their extension. All of it is parsed in the app itself — no new dependency, and the DEFLATE/ZIP/XML readers are unit-tested against real files.
>   - Desktop gets a library rail beside the document; phones get a single pane that swaps between the two.
